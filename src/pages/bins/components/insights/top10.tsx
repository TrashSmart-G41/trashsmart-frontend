...
export function TopTen({ organizations }) {
  ...
  const sortedOrganizations = organizations
    .map((org, index) => ({
      ...org,
      rank: index + 1,
      noOfBins: org.commercialBins?.length || 0,
      wasteAccumulation: org.weeklyWaste || 0,
      logo,
    }))
    .sort((a, b) => b.noOfBins - a.noOfBins)
            ...
            <TableRow key={org.rank}>
              ...
              <TableCell>
                <div className='flex items-center space-x-4'>
                  ...
                  {/*  {org.logo ? (*/}
                  {/*    <img src={org.logo} alt={`${org.name} logo`} className='w-12 h-12 object-cover rounded-full' />*/}
                  {/*  ) : (*/}
                  {/*    <div className='w-12 h-12 bg-gray-300 rounded-full flex items-center justify-center'>*/}
